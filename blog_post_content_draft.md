...
4.  **Stream the Shell:** The browser receives this initial HTML, including any server-rendered fallbacks for suspended components, and can start rendering it immediately. This gives the user a fast initial view.
...
`[CODE_EXAMPLE_DATA_FETCHING_SETUP]`

In this scenario, `UserWelcomeMessage` is a Server Component that fetches data.
```jsx
// app/components/UserWelcomeMessage.js
...
export default async function UserWelcomeMessage() {
  const userData = await getUserData();
  return <h1>Welcome, {userData.name}!</h1>;
}
```

Your page component would use Suspense to show a loading state:
```jsx
// app/dashboard/page.js
import { Suspense } from 'react';
import UserWelcomeMessage from '../components/UserWelcomeMessage';
import WelcomeSkeleton from '../components/WelcomeSkeleton'; // Your fallback UI
...
export default function DashboardPage() {
  return (
    <div>
      ...
      {/* Other dashboard content that loads immediately */}
      <p>Your main dashboard content that isn't waiting for the welcome message.</p>
    </div>
  );
}

// app/components/WelcomeSkeleton.js
export default function WelcomeSkeleton() {
  return <h1>Loading user information...</h1>;
}
```
`[CODE_EXAMPLE_DATA_FETCHING]`
...
*   **The `loading.js` Convention in Next.js:**
    ...
    ```
    With this file in place, you often don't need to manually add `<Suspense>` in your `page.js` for the primary page content if the `loading.js` fallback is sufficient.
...
*   **Scenario:** You have a feature-rich, but heavy, charting component that is only shown when a user clicks a button. It doesn't make sense to bundle this component with the initial page load.

`[CODE_EXAMPLE_LAZY_LOADING_SETUP]`
```jsx
// components/HeavyChartingComponent.js
// Assume this component is large and uses 'use client'
'use client';
import React from 'react';

export default function HeavyChartingComponent({ data }) {
  // Complex charting logic here
  return <div>Displaying complex chart with data: {JSON.stringify(data)}</div>;
}

// app/reports/page.js
'use client'; // This page needs to be a client component to use useState and React.lazy

import React, { Suspense, useState, lazy } from 'react';

// Lazy load the heavy component
const LazyHeavyChartingComponent = lazy(() => import('../../components/HeavyChartingComponent'));

export default function ReportsPage() {
  const [showChart, setShowChart] = useState(false);
  const chartData = { value: 42 }; // Example data

  return (
    <div>
      <h2>Reports</h2>
      <button onClick={() => setShowChart(true)}>Show Detailed Chart</button>
      {showChart && (
        <Suspense fallback={<div>Loading chart module...</div>}>
          <LazyHeavyChartingComponent data={chartData} />
        </Suspense>
      )}
      <p>Other report content that loads immediately.</p>
    </div>
  );
}
```
`[CODE_EXAMPLE_LAZY_LOADING]`

In this example, the JavaScript for `HeavyChartingComponent` will only be downloaded by the browser when `showChart` becomes true. During the download and preparation of the component, the `fallback` UI ("Loading chart module...") is displayed. This keeps your initial bundle size smaller and improves load times for users who don't immediately need the heavy component.
